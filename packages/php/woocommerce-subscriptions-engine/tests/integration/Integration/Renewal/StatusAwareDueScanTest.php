...
class StatusAwareDueScanTest extends EngineIntegrationTestCase {
	...
	/**
	 * Seed a contract at a status with a due (past) next-payment date + an AS row.
	 *
	...
	private function seed_due( string $status ): int {
		...
		RenewalScheduler::schedule( $id, new \DateTimeImmutable( '2026-02-01 00:00:00', new \DateTimeZone( 'UTC' ) ) );
	...
	public function test_on_hold_contract_at_its_date_creates_no_renewal_order(): void {
		...
		$result = $this->engine->process_due( $id );

		$this->assertNull( $result );
		$this->assertSame( 0, $this->renewal_order_count( $id ) );
		// Still on hold; the row is left for reactivate to re-arm.
