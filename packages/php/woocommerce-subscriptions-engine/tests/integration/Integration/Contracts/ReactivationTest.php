<?php
/**
 * Integration tests for the Reactivation contract operation: ON_HOLD -> ACTIVE, the
 * next-payment date recomputed forward (the Model-1 seam), and the renewal re-armed.
 *
...
class ReactivationTest extends EngineIntegrationTestCase {
	...
	public function set_up(): void {
		...
		// Reactivate re-arms via RenewalEngine::schedule(), which gates on the gateway's
		// recurring capability; declare it so the re-arm sticks.
	...
	public function test_reactivate_resumes_and_rearms_the_renewal(): void {
		...
		$this->assertTrue( RenewalScheduler::is_scheduled( $id ), 'Reactivate re-arms the schedule.' );
	}
	...
	public function test_reactivate_leaves_a_null_next_payment_null_and_unscheduled(): void {
		...
		$this->assertFalse( RenewalScheduler::is_scheduled( $id ) );
	}
