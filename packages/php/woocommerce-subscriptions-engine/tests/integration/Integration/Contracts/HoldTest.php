<?php
/**
 * Integration tests for the Hold contract operation: ACTIVE -> ON_HOLD, the pending
 * renewal cleared, and the held action fired.
 *
...
class HoldTest extends EngineIntegrationTestCase {
	...
	/**
	 * Seed a contract at a status with a future next-payment date and a pending AS row.
	 *
	...
	public function test_hold_suspends_and_clears_the_scheduled_renewal(): void {
		...
		$this->assertFalse( RenewalScheduler::is_scheduled( $id ), 'No charge while held: the AS row is cleared.' );
	}
