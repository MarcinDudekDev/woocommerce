 ...
 * PENDING_CANCELLATION, the end date stamped, and the renewal left armed so the
 * contract lapses at period end.
 *
...
class PeriodEndCancellationTest extends EngineIntegrationTestCase {
	...
	/**
	 * Seed an active contract with a future next-payment date and a pending AS row.
	 *
	...
	private function seed_active( ?string $end_gmt = null ): int {
		...
		RenewalScheduler::schedule( $id, new \DateTimeImmutable( '2099-01-01 00:00:00', new \DateTimeZone( 'UTC' ) ) );
		$this->assertTrue( RenewalScheduler::is_scheduled( $id ) );
	...
	public function test_leaves_the_renewal_armed_so_the_contract_lapses_at_the_date(): void {
		// The pending AS row is deliberately left; the due scan refuses to charge a
		// non-active contract, so no renewal fires while it winds down.
		$id = $this->seed_active();

		$this->sut->cancel_at_period_end( $this->reload( $id ) );

		$this->assertTrue( RenewalScheduler::is_scheduled( $id ), 'The contract lapses at the date; the row stays armed.' );
	}
