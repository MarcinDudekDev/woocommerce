...
final class ContractsController extends WP_REST_Controller {
	...
	public function get_items( $request ): WP_REST_Response {
		...
		$per_page = $this->clamp_limit( self::coerce_int( $request['per_page'] ) );
		$page     = max( 1, self::coerce_int( $request['page'] ) );
	...
	public function get_item( $request ) {
		$contract = $this->owned_contract( self::coerce_int( $request['id'] ) );
	...
	public function cancel_item( $request ) {
		...
		$at_period_end = rest_sanitize_boolean( self::coerce_string( $request['at_period_end'], 'true' ) );
