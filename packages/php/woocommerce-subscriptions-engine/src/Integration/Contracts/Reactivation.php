 ...
 * Core state machine, recompute the next-payment date forward, re-arm the renewal, and
 * announce it. Lives under `Integration\Contracts` so contract lifecycle stays separate
 * from the renewal money-path.
 *
...
final class Reactivation {
	 ...
	 */
	const CONTRACT_REACTIVATED_ACTION = 'woocommerce_subscriptions_engine_contract_reactivated';
	...
	/**
	 * Renewal engine, for re-arming the schedule.
	 *
	 * @var RenewalEngine
	 */
	private $renewals;
	 ...
	 * @param RenewalEngine|null      $renewals  Renewal engine; default instance when omitted.
	 */
	public function __construct( ?ContractRepository $contracts = null, ?PlanRepository $plans = null, ?RenewalEngine $renewals = null ) {
		$this->contracts = $contracts ?? new ContractRepository();
		$this->plans     = $plans ?? new PlanRepository();
		$this->renewals  = $renewals ?? new RenewalEngine( $this->contracts, $this->plans );
	}

	/**
	 * Reactivate `$contract`: transition to active, recompute the next-payment date
	 * forward, persist, and re-arm the renewal.
	 *
	 ...
	 * date does not fire an immediate, back-dated renewal the moment it resumes.
	 * {@see RenewalEngine::schedule()} then no-ops when there is no date to arm.
	 *
	...
	public function reactivate( Contract $contract, ?DateTimeImmutable $now = null ): bool {
		...
		// Re-arm at the recomputed forward date (schedule() no-ops when there is none).
		$this->renewals->schedule( $contract );
