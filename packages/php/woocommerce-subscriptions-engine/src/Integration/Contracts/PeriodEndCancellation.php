 ...
 * next-payment moment, keep the pending renewal scheduled so the contract lapses at
 * period end, and announce it. For an immediate cancel, use {@see Cancellation} instead.
...
final class PeriodEndCancellation {
	 ...
	 */
	const CONTRACT_PENDING_CANCELLATION_ACTION = 'woocommerce_subscriptions_engine_contract_pending_cancellation';
	 ...
	 * the pending Action Scheduler row is deliberately LEFT in place so the contract
	 * lapses at the date rather than being torn down now.
	 *
	 * The due scan already refuses to charge a non-active contract ({@see RenewalEngine::process_due()}
	...
	public function cancel_at_period_end( Contract $contract ): bool {
		...
		// Intentionally do NOT unschedule: the contract lapses at the date (see the TODO above).
